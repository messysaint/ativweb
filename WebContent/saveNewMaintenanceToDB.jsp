...
<body>
	...
	String sessiontimeout = "/ivweb/z_securityquery.jsp";
 	...
 	if( !check.isLoggedIn( IsHuman ) ) {	// if not logged in, ask user to log in
 		...
 		String cost = request.getParameter("costofservice");
 		...
 		while (enumPar.hasMoreElements()) {
 			...
 			if (objName.startsWith("dateofservice") ) {
 				...
 				servicesPerformed += request.getParameter(objName) + ".  ";
 			}

 		}

 		servicesPerformed += " [Notes: " + notes + "]";	
 		String vin = IVWEB_VIN; 
 		
 		saveNewMaintenanceRecord newMtnc = new saveNewMaintenanceRecord();
 		boolean saveOk = newMtnc.WriteToDB( "100", vin, date, cost, odometer, servicesPerformed );
 		newMtnc.CloseDB();
